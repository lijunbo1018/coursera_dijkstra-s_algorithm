...
{
	int range = (int)upper * 10 - (int)lower * 10;            //to get a random decimal 
...
float Graph::get_shortest_path(int s, int t)
{
	...
	{
		cout << "No Such Start Vectex " << s <<endl;
		return 0.0;
	}
	if ((t < 1) || (t > number_of_vertices))
	{
		cout << "No Such End Vectex " << t <<endl;
		return 0.0;
	}
	if (s == t)
	{
	...
	cout << "The Shortest Path Form " << s << " To " << t << " is: " << endl;
	...
	cout << "which cost is " << T.cost_from_start << endl;
	open_set.clear();
	closed_set.clear();
